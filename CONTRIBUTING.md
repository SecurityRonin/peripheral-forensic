# Contributing to peripheral-forensic

Thanks for your interest in improving `peripheral-forensic`. This crate parses
untrusted Windows device-installation logs from potentially compromised systems,
so correctness and robustness are not negotiable. The bar is high and the workflow
is strict — please read this before opening a pull request.
...
against **real external data** where it matters: cross-check parsing against a
genuine `setupapi.dev.log` copied from a Windows host, not only synthetic
fixtures.
...
cargo llvm-cov --workspace --lib --show-missing-lines   # 100% production-line coverage
```
...
- **Coverage** — every production line is covered, except genuinely-unreachable
  defensive guards annotated `// cov:unreachable`. New code needs tests that
  exercise its error paths, not just the happy path.
- **Fuzzing** — changes to the parser must keep its fuzz target green. Run the
  relevant target before submitting:

  ```bash
  rustup install nightly
  cargo install cargo-fuzz
  cargo +nightly fuzz run setupapi     # or: forensic
  ```

  If the parser gains new structure handling, extend or add the matching fuzz
  target.

## Robustness expectations

- No panics on malicious input — parsing is lenient (lossy UTF-8) and
  bounds-checked; a truncated or garbled log degrades line-by-line, never a
